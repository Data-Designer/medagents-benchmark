    ...
    for model in o1-mini o3-mini; do
        {
            for dataset in medqa pubmedqa medmcqa medbullets mmlu mmlu-pro medexqa medxpertqa-r medxpertqa-u; do
                ...
                for split in test_hard; do
                    ...
                    python few_shot.py --dataset_name $dataset --dataset_dir $DATA_DIR/$dataset/ --split $split --model $model --output_files_folder ../../output/run-${RUN_NUMBER}/ --num_processes 4 > $log_file 2> $error_file
                done
            done
        } &
    done
